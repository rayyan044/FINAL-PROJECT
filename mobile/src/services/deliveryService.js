...
export async function getDeliveryNote(deliveryId) {
  try {
    const response = await api.get(`/mobile/deliveries/${deliveryId}/delivery-note`);
    if (!response.data?.data || typeof response.data.data !== "object") {
      throw new Error("The delivery note service returned an unexpected response.");
    }
    return response.data.data;
  } catch (error) {
    if (error.message === "The delivery note service returned an unexpected response.") throw error;
    if (error.response?.status === 400) {
      throw new Error(error.response.data?.message || "The delivery note has not been generated yet.");
    }
    throw new Error(getErrorMessage(error));
  }
}

export async function downloadDeliveryNotePdf(deliveryId) {
  try {
    const response = await api.get(`/mobile/deliveries/${deliveryId}/delivery-note/pdf`, { responseType: "arraybuffer" });
    return response.data;
  } catch (error) {
    throw new Error(getErrorMessage(error));
  }
