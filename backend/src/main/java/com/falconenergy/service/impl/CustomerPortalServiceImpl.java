...
public class CustomerPortalServiceImpl implements CustomerPortalService {
    ...
    @Override public CustomerPortalResponse.Order createOrder(CustomerPortalOrderRequest r) {
        ...
        FuelOrderRequest request = FuelOrderRequest.builder().orderNumber("ORD-" + UUID.randomUUID().toString().substring(0,8).toUpperCase())
                ...
                .locationAddress(r.getDeliveryAddress()).locationLandmark(r.getLocationLandmark()).paymentMethod(r.getPaymentMethod()).notes(r.getNotes()).destination(r.getDestination()).deliveryLatitude(r.getDeliveryLatitude()).deliveryLongitude(r.getDeliveryLongitude()).build();
    ...
    private BigDecimal orderTotalBeforeTax(FuelOrder o) { return money(o.getAmount()).add(money(o.getTransportCharges())).add(money(o.getDeliveryCharges())).add(money(o.getAdditionalCharges())).add(money(o.getLevies())).subtract(money(o.getDiscount())); }
    private BigDecimal orderTotalBeforeTax(FuelOrderResponse o) { return money(o.getAmount()).add(money(o.getTransportCharges())).add(money(o.getDeliveryCharges())).add(money(o.getAdditionalCharges())).add(money(o.getLevies())).subtract(money(o.getDiscount())); }
    ...
    @Override @Transactional(readOnly=true) public CustomerPortalResponse.DeliveryTracking deliveryTracking(Long id){
        Customer c=customer();
        Delivery d=deliveryRepository.findForCustomerById(id,c.getId()).orElseThrow(()->new ResourceNotFoundException("Delivery not found."));
        boolean live=d.getDeliveryStatus()==DeliveryStatus.IN_TRANSIT && d.getCurrentLatitude()!=null && d.getCurrentLongitude()!=null && d.getLocationUpdatedAt()!=null;
        return CustomerPortalResponse.DeliveryTracking.builder().deliveryId(d.getId()).deliveryNumber(d.getDeliveryNumber()).truckNumber(d.getTruckNumber()).status(String.valueOf(d.getDeliveryStatus())).live(live).latitude(live?d.getCurrentLatitude():null).longitude(live?d.getCurrentLongitude():null).accuracy(live?d.getCurrentLocationAccuracy():null).updatedAt(live?d.getLocationUpdatedAt():null).build();
    }
