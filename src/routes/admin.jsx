...
function AdminDash() {
  ...
  useEffect(() => {
    if (activeTab === "audit") {
      ...
      loadInvoices();
    } else if (activeTab === "paymentAccounts") {
      loadPaymentAccounts();
    } else if (activeTab === "companySettings") {
      loadCompanySettings();
    } else {
      loadData();
    }
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (
      targetUser &&
      currentLoggedUser &&
      (targetUser.username === currentLoggedUser.username ||
        targetUser.email === currentLoggedUser.email)
    ) {
    ...
    if (
      !window.confirm("Are you sure you want to delete this user? This will perform a soft-delete.")
    )
      return;
  ...
  const handleToggleStatus = async (u) => {
    ...
    if (
      currentLoggedUser &&
      (u.username === currentLoggedUser.username || u.email === currentLoggedUser.email)
    ) {
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ textAlign: "right" }}>
                          <strong>{stats.users - stats.salesOfficers}</strong>
                        </td>
                      </tr>
                      <tr>
                        <td>SALES_OFFICER</td>
                        <td style={{ textAlign: "right" }}>
                          <strong>{stats.salesOfficers}</strong>
                        </td>
                      </tr>
                      <tr>
              ...
              <div className="fef-panel">
                ...
                <div style={{ padding: 20 }}>
                  <div style={{ marginBottom: 12 }}>
                    <span style={{ color: "var(--feftms-text-muted)" }}>
                      Fuel Requests (Orders):
                    </span>{" "}
                  ...
                  <div className="fef-form-grid">
                    <div className="fef-field">
                      ...
                      <input
                        ...
                        disabled={editId && editId === currentLoggedUser?.id}
                      />
                    </div>
                    <div className="fef-field">
            ...
            {resetUser && (
              <div
                ...
                style={{
                  padding: 20,
                  marginBottom: 24,
                  border: "1px solid var(--feftms-secondary)",
              ...
              <table className="fef-table">
                ...
                <tbody>
                  {usersList.map((u) => {
                    const isSelf =
                      currentLoggedUser &&
                      (u.username === currentLoggedUser.username ||
                        u.email === currentLoggedUser.email);
                    return (
                      <tr key={u.id}>
                        <td>
                          <strong>{u.username}</strong>{" "}
                          {isSelf && <span style={{ fontSize: 10, opacity: 0.6 }}>(You)</span>}
                        </td>
                        <td>
                        ...
                        <td>{u.phone || "—"}</td>
                        <td>
                          <span className="fef-badge fef-badge-progress">{u.role}</span>
                        </td>
                        <td>
                          <button
        ...
        {activeTab === "requests" && (
          <div className="fef-panel">
            <div style={{ padding: "20px 0", color: "var(--feftms-text-muted)" }}>
              Please approve or reject customer requests in the{" "}
              <strong>Sales Desk Dashboard</strong>. Complete logs are displayed below.
            </div>
            <div className="fef-table-wrap">
              <table className="fef-table">
                <thead>
            ...
            >
              <div>
                ...
                <p style={{ margin: 0, color: "var(--feftms-text-muted)" }}>
                  Configure bank accounts and payment instructions dynamically loaded on invoices.
                </p>
              </div>
              <button
                className="fef-btn fef-btn-primary"
                  ...
                  {paymentAccounts.length === 0 && (
                    <tr>
                      <td
                        ...
                        style={{ textAlign: "center", color: "var(--feftms-text-muted)" }}
                      >
                        No payment accounts configured. Click "+ Add Payment Account" to get
                        started.
                      </td>
                    </tr>
                  )}
