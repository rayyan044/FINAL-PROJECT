...
function ViewerDash() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.allSettled([listAuditLogs({ page: 0, size: 10 }), listTransactions({ page: 0, size: 10 })])
      .then((results) => {
        if (results[0].status === "fulfilled") setAudits(results[0].value.content || results[0].value || []);
        if (results[1].status === "fulfilled") setTransactions(results[1].value.content || results[1].value || []);
  ...
  return (
    ...
    >
      ...
      <div className="fef-stat-grid">
        <StatCard label="Recent Transactions" value={loading ? "…" : String(transactions.length)} icon={FiTrendingUp} tone="primary" />
        ...
        <div style={{ padding: 20, color: "var(--feftms-text-muted)" }}>
          {loading ? (
            ...
            <div>
              ...
              <ul>
                {audits.map((a) => (
                  <li key={a.id}>{a.action} — {a.entity} — {a.createdAt}</li>
                ))}
              </ul>
              ...
              <ul>
                {transactions.map((t) => (
                  <li key={t.id}>{t.transactionNumber || t.id} — {t.transactionType} — {t.quantity}</li>
                ))}
              </ul>
            </div>
          )}
